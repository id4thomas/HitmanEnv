...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_mem', type=int, default=1000,
                    help='Max Replay memory size')
    parser.add_argument('--ver', default='1',
                    help='hitman version (1~4)')

    args = parser.parse_args()

    ver = args.ver

    version = 'hitman-v' + ver
    ...
    if not os.path.exists('weight_' + ver):
        os.makedirs('weight_' + ver)
    ...
    for ep_i in range(num_episodes):
        ...
        while not done:
            ...
            replay_memory_append(replay_memory, [previous_obs, action, reward, obs, done], args.max_mem)
        ...
        print('Episode #{} total reward: {} step: {} epsilon {} path{}: '.format(ep_i, cnt, step_count,
                                                                                 main_network.get_epsilon(), path))
        ...
        if ep_i % 50 == 0 and ep_i != 0:
            main_network.save_model('./weight_' + ver + '/model_ep{}.h5'.format(ep_i))
